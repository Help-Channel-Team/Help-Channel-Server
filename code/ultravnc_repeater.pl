...
sub ConnectToDB{
	$dbh = DBI->connect('DBI:mysql:helpchannel;host=127.0.0.1', 'USER', 'PASSWORD',{ RaiseError => 1 });
}

# Database operations to update connection status
sub InitConnection{
        # Sets the connection status to Stablished
        ConnectToDB();
	$dbh->do('UPDATE `helpchannel`.`helpchannel_connection` SET status_id=4, modification_date=NOW(), start_date=NOW() WHERE connection_code=?;', undef, @_);
	$dbh->disconnect();
}

sub CloseConnectionUser{
        # Sets the connection status to Finished User
	ConnectToDB();
        $dbh->do('UPDATE `helpchannel`.`helpchannel_connection` SET status_id=6, modification_date=NOW(), end_date=NOW() WHERE connection_code=?;', undef, @_);
	$dbh->disconnect();
}

sub CloseConnectionTech{
        # Sets the connection status to Finished Tech
        ConnectToDB();
	$dbh->do('UPDATE `helpchannel`.`helpchannel_connection` SET status_id=9, modification_date=NOW(), end_date=NOW() WHERE connection_code=?;', undef, @_);
	$dbh->disconnect();
}

sub AbortConnection{
        # Sets the connection status to Aborted
        ConnectToDB();
	$dbh->do('UPDATE `helpchannel`.`helpchannel_connection` SET status_id=10, modification_date=NOW(), end_date=NOW() WHERE connection_code=?;', undef, @_);
	$dbh->disconnect();
}
	...
	if ($buf =~ /^ID:(\w+)/) {
		...
		if (exists $ID{$id}) {
			if (! $ID{$id}{client}) {
				...
				if ($ref) {
					lprint("client - refusing extra vnc server for ID:$id.");
					close $sock;
					return;
				} else {
					lprint("client - closing and deleting previous vnc server with ID:$id.");
					close $ID{$id}{sock};

					lprint("client - storing new vnc server with ID:$id.");
					$ID{$id}{client} = 0;
					$ID{$id}{sock} = $sock;
				}
			} else {
				lprint("client - hooking up new vnc server with existing vnc client for ID:$id.");
				my $sock2 = $ID{$id}{sock};
				delete $ID{$id};
				hookup($sock, $sock2, "ID:$id"); 
			}
		...
		lprint("client - invalid ID:NNNNN string for vnc server: $buf");
		close $sock;
		return;
	}
}
